...
class clinicalVeterinarian extends Component {
    ...
    render() {
        ...
        if (stories){
            requiredStories = stories.filter(stories => stories.selectCategory.find(element=>element==="5c59f3f843a88f39f9abdcb1"))
        }
        ...
        if(pages!== null && pages.length>0){
            pages.forEach((item,index)=>{
                if(item.templateOrder==='one' && item.type === "stories"){
                    one =[<StoriesHeader publicationCategory={"Equine Athlete"} key={index} headerImg={item && item.featuredImage && item.featuredImage.url} heading={"Stories"}/>]
                }
        ...
        return (
            <div>
                ...
                {requiredStories !==null && requiredStories.length>0 && <StoriesCategory stories={requiredStories} storiesCategory={"Equine Athlete"}/>}
                <RequestDemo/>
            </div>
        )
    }
