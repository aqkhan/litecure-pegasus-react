...
class webinarsCategoty extends Component {
    ...
    async componentWillReceiveProps(nextProps, nextContext) {
        let {webinarCategory, webinars, page} = nextProps;
        await this.setState({webinarCategory:webinarCategory,webinars:webinars, page})
    ...
    setDisplay(){
        ...
        let temp=[];
        this.state.webinars.forEach((val)=>{
            let check = 0;
        ...
        this.setState({webinarDestructure:restructured, final: restructured[0], totalPages: restructured.length}, function () {
    ...
    returnData = (index) => {
        ...
        this.setState({final: this.state.webinarDestructure[index.selected]})
    }
    ...
    render(){
        let {webinarCategory, page, final,totalPages, one, two, three, four, five, six, seven, eight, nine, ten , eleven, twelve} = this.state;
        let cards;
        if(final){
